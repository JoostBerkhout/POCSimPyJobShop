...
@pytest.fixture
def elite_set():
    """Fixture for initializing EliteSet."""
    return EliteSet()


def test_add_solution(elite_set, mock_solution, mock_simulator):
    """Test adding a solution."""
    schedule = {0: [1, 2], 1: [3]}
    assert elite_set.is_new_schedule(schedule)
    elite_set.add(
        mock_solution, mock_simulator, objective=25.0, schedule=schedule
    )
    assert not elite_set.is_new_schedule(schedule)
    assert len(elite_set) == 1
    assert elite_set[0].schedule == schedule
    assert elite_set[0].objective == 25.0
    assert elite_set[0].simulator == mock_simulator


def test_keep_top_n(elite_set):
    ...
    for i in range(5):
        ...
        simulator.num_sims = 10
        elite_set.add(solution, simulator, 10, {0: [0, 1]})

    assert len(elite_set) == 5

    # Keep top 3 solutions
    elite_set.keep_top_n(3)
    assert len(elite_set) == 3

    # Check that the solutions with the lowest mean remain
    remaining_means = [sol.simulator.mean for sol in elite_set]
    assert sorted(remaining_means) == [46.0, 47.0, 48.0]  # Lowest means

    # Keep top 3 solutions (nothing should happen)
    elite_set.keep_top_n(3)
    assert len(elite_set) == 3
    remaining_means = [sol.simulator.mean for sol in elite_set]
    assert sorted(remaining_means) == [46.0, 47.0, 48.0]  # Lowest means

    # Keep top 2 solutions
    elite_set.keep_top_n(2)
    assert len(elite_set) == 2
    remaining_means = [sol.simulator.mean for sol in elite_set]
    assert sorted(remaining_means) == [46.0, 47.0]  # Lowest means


def test_get_best_solution(elite_set, mock_solution, mock_simulator):
    ...
    for i in range(3):
        ...
        simulator.num_sims = 10
        elite_set.add(solution, simulator, 10.0, {0: [0, 1]})

    # Add the best solution
    elite_set.add(best_solution, best_simulator, 5.0, {0: [0, 1]})

    assert elite_set.get_best_mean_solution().solution == best_solution


def test_get_worst_solution(elite_set, mock_solution, mock_simulator):
    ...
    worst_solution = elite_set.get_worst_mean_solution().solution

    assert worst_solution == solution


def test_is_new_schedule(elite_set, mock_solution, mock_simulator):
    """Test checking if a schedule is new."""
    schedule = {0: [1, 2], 1: [3]}
    elite_set.add(mock_solution, mock_simulator, 25.0, schedule)

    new_schedule = {0: [2, 3], 1: [4]}
    assert elite_set.is_new_schedule(new_schedule) is True

    duplicate_schedule = {0: [1, 2], 1: [3]}
    assert elite_set.is_new_schedule(duplicate_schedule) is False


def test_print_summary(capsys, elite_set, mock_solution, mock_simulator):
...
def test_simulate_to_num_sims(elite_set, mock_solution, mock_simulator):
    ...
    num_sims = 5
    elite_set.simulate_to_num_sims(num_sims)
    ...
    elite_set.simulate_to_num_sims(num_sims, time_limit)
    assert mock_simulator.simulate.call_count == 1
...
def test_sim_to_time_limit(elite_set):
    ...
    elite_set.add(solution1, simulator1, 25.0, {0: [1, 2]})
    elite_set.add(solution2, simulator2, 30.0, {0: [3, 4]})

    # Test 1: Only time to simulate the second solution 3 times
    start_time = time.time()
    elite_set.simulate_to_time_limit(0.15)
    ...
    start_time = time.time()
    elite_set.simulate_to_time_limit(0.1)
    duration = time.time() - start_time
